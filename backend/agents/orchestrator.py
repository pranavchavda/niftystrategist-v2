...
- `python cli-tools/nf-deploy-sessions [--universe U --top N | --plan-file F.json] [--days 15] [--max-sessions 12] [--min-confidence high|medium|low] [--risk-per-trade RS | --capital-per-trade RS | --quantity N] [--execute] [--json]` — Turns a signal-match scan into live `nf-scalp` sessions, deploying the matched indicator + direction per stock. **Dry-run by default — previews the plan; `--execute` is REQUIRED to actually create sessions.** Applies concurrency (`--max-sessions`) and indicator-family diversification caps (`--max-family-fraction`, default 0.4). Sizes each session from a LIVE LTP via capital/risk/fixed. ALWAYS preview (no `--execute`) and show the user the plan before executing — this places live algorithmic sessions. Backtest stats are in-sample (optimistic). Note: no per-sector diversification (no sector map exists yet).
