...
async def create_competitor(competitor_data: CompetitorCreate):
    ...
    try:
        competitor = await competitor_service.create_competitor(competitor_data.model_dump())
        return competitor
...
async def update_competitor(competitor_id: str, competitor_data: CompetitorUpdate):
    ...
    try:
        ...
        update_data = {k: v for k, v in competitor_data.model_dump().items() if v is not None}
...
@router.post("/product-matching/verify")
async def verify_product_match(
    match_id: str = Body(...),
    verified_by: Optional[str] = Body(None)
):
    """Verify/approve a product match, converting it to a manual match"""
    try:
        result = await matching_service.verify_match(match_id, verified_by)
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        logger.error(f"Error verifying product match {match_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to verify product match")

@router.post("/product-matching/reject")
async def reject_product_match(
    match_id: str = Body(...),
    reason: Optional[str] = Body(None),
    rejected_by: Optional[str] = Body(None)
):
    """Reject a product match and blacklist the pair from future auto-matching"""
    try:
        result = await matching_service.reject_match(match_id, reason, rejected_by)
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        logger.error(f"Error rejecting product match {match_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to reject product match")
...
async def get_alerts(
    ...
    try:
        ...
        from database.price_monitor_models import PriceAlert, ProductMatch, IdcProduct, CompetitorProduct, Competitor
    ...
    try:
        ...
        if format == 'csv':
            ...
            return Response(
                content=export_data,
                media_type='text/csv',
                headers={"Content-Disposition": f"attachment; filename=violation-history-{utc_now_naive().strftime('%Y-%m-%d')}.csv"}
            )
        else:
            return export_data
    except Exception as e:
...
async def scan_map_violations(
    ...
    try:
        result = await violations_service.scan_and_record_violations(
            ...
            severity_filter=request.severity_filter,
            record_history=request.record_history,
            create_alerts=getattr(request, 'create_alerts', True),  # Default to True if not provided
            capture_screenshots=request.capture_screenshots,
            dry_run=request.dry_run
        )
        return result
...
async def get_map_violations(
    ...
    try:
        ...
        from database.price_monitor_models import ProductMatch, IdcProduct, CompetitorProduct, Competitor
        from sqlalchemy.orm import selectinload
        ...
        for match in matches:
            if match.is_map_violation and match.idc_product and match.competitor_product:
                ...
                violations_list.append({
                    'id': match.id,
                    'product_match_id': match.id,  # Add top-level product_match_id for easy access
                    'severity': severity,
...
@router.post("/map-violations/violations/resolve")
async def resolve_violation(
    request: Request,
    db: AsyncSession = Depends(get_db)
):
    """Resolve a MAP violation by marking it as no longer violating"""
    try:
        from sqlalchemy import select, update
        from database.price_monitor_models import ProductMatch, ViolationHistory
        from datetime import datetime
        import uuid
        ...
        # Find the product match (violation_id is actually the product_match_id)
        from sqlalchemy.orm import selectinload
        from database.price_monitor_models import IdcProduct, CompetitorProduct

        result = await db.execute(
            select(ProductMatch)
            .options(
                selectinload(ProductMatch.idc_product),
                selectinload(ProductMatch.competitor_product)
            )
            .where(ProductMatch.id == violation_id)
        )
        ...
        if not match.is_map_violation:
            return {"message": "Violation already resolved", "violation_id": violation_id}

        # Record the resolution in violation history
        history_entry = ViolationHistory(
            id=str(uuid.uuid4()),
            product_match_id=violation_id,
            violation_type="resolved",
            competitor_price=match.competitor_product.price if match.competitor_product else 0,
            idc_price=match.idc_product.price if match.idc_product else 0,
            violation_amount=match.violation_amount or 0,
            violation_percent=match.violation_percentage or 0,
        ...
        from database.price_monitor_models import PriceAlert
        alert_result = await db.execute(
            select(PriceAlert).where(
                PriceAlert.product_match_id == violation_id,
